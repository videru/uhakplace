<?
	...
	require_once("admin_chk.php");
		...
		$comm_date = mktime(0,0,0,$paid2,$paid3,$paid1);
	      ...
	      if($student_no>0){
	    	...
	    	$rs_list->add_field("comm_date1","$paid1");	
	    	$rs_list->add_field("comm_date2","$paid2");	
    	    ...
    	    $rs->add_field("comm_date1","$paid1");	
    	    $rs->add_field("comm_date2","$paid2");				
	    	$rs->add_field("order","$order");	
	    	$rs->add_field("etc","$etc");			
			$rs->insert();
			$ac_no=$rs->get_insert_id();			
            $rs->commit();
