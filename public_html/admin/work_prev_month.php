...
<tr><td align="center">
<a href="person_check_list.php?month=<?=$prevmonth?>"><span style="font-size:10pt;font-weight:bold;"><?=$prevmonth?>월</span></a> <span style="font-size:8pt;"><?=$prevyear?></span>
</td><tr>
	<tr>
		<td>
							...
							<tr>
								<?
									...
									for($day=1;$day<=$total_day;$day++){
										...
										$cal_list->clear();
										$cal_list->set_table($_table['consult']);
										$cal_list->add_where("regi_date='$view_date'");
										$cal_list->add_order("c_no ASC"); 
										if($cal=$cal_list->fetch()){
											echo "<A HREF='person_check_edit.php?c_no={$cal[c_no]}&year=$prevyear&month=$prevmonth' title='$cal[bd_subject]'><b><u><span style=' color:$day_color;'>$day</span></u></b></a></td>" ;
										} else {
