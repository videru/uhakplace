function Folder(folderDescription, hreference, folderOpenImg, folderCloseImg, folderKey) {
   //constant data;
   ...
   this.nodeImg = 0;
   this.isLastNode = 0;
   //this.openimg = folderOpenImg;
   //this.closeimg = folderCloseImg;

   // dynamic data
   this.isRoot = true;
   this.isOpen = true;
   this.iconSrc = folderOpenImg;
   this.iconRoot = folderCloseImg;
...
function propagateChangesInState(folder) {   
   ...
   if(folder.isOpen) { 
       ...
       if(this.isRoot)
           folder.iconImg.src = "images/mail.gif";
       else 
       ...
       if(this.isRoot)
           ...
           folder.iconImg.src = folder.iconRoot;
...
function drawFolder(leftSide) { 
    ...
    if(leftSide == '') {
        doc.write("SRC='images/" + this.iconRoot+"' align='absmiddle' BORDER=0></A>");
    }
    else {
        doc.write("SRC='images/" + this.iconSrc+"' align='absmiddle' BORDER=0></A>");
    }
       
    doc.write("</TD><TD NOWRAP>");
  
    doc.write("<DIV CLASS=\"linkfolder\">");
    ...
    if(leftSide == '') { 
       if(USETEXTLINKS) {
        ...
        doc.write("<A HREF='JavaScript: clickOnNode("+this.id+")' CLASS=\"linkfolder\">");
        doc.write(this.desc + "</A>");
    }
...
function Item(itemDescription, hreference, itemLink, itemImg, itemKey) // Constructor 
{ 
    // constant data 
...
function drawItem(leftSide) { 
    ...
    if(USETEXTLINKS) {
        if(this.link != "")
            doc.write("<A HREF=" + this.link + " CLASS=\"linkmenu\">" + this.desc + "</A>");
        else
            doc.write(this.desc);
    }
    else {
...
function clickOnNode(folderId) { 
    ...
    rootFolder = indexOfEntries[0];
    clickedFolder = indexOfEntries[folderId];
    state = clickedFolder.isOpen;
    rootState = rootFolder.isOpen;

	//clickedFolder.setState(!state); //open<->close
	curkey = clickedFolder.key;
	
	if (mspr == "0") {
		clickedFolder.setState(!state); //open<->close
	} else {
		if (clickedFolder.key == "SubFolder") {
			clickedFolder.setState(!state); //open<->close
		} else {
			rootFolder.setState(0);
			rootFolder.setState(1);
...
function gFldr(description, hreference, folderOpenImg, folderCloseImg, folderKey) { 
    folder = new Folder(description, hreference, folderOpenImg, folderCloseImg, folderKey);
    return(folder);
}


function gLnk(target, description, linkData, itemImg, itemKey) { 
    fullLink = "" 
    ...
    linkItem = new Item(description, linkData, fullLink, itemImg, itemKey)   
    return linkItem 
} 
