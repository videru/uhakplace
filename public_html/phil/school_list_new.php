...
<?
    ...
    {
    	$count = count($_const['statecitynum'][$national][$state]);
    	...
    	if($count>0)
    	{
	    	...
	    	for ($i = 0; $i < $count; $i++) {
	    		...
	    		$where.="area=".$_const['statecitynum'][$national][$state][$i];
	    		if($i < $count-1)
	    			$where.=" or ";
	    		}
...
<table width="520"  align="center">
	<tr>
		<td align="center">
      <?=rg_navi_display($page_info,"&national={$national}&state={$state}&area={$area}"); ?>
		</td>
	</tr>
</table>
