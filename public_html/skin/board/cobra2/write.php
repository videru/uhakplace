...
?>
<form name="add_form" method="post" action="?" onSubmit="return validate(this)" enctype="multipart/form-data">
<?=$_post_param[3]?>
...
<? if($_mb[mb_level]>9) { ?>
	<tr>
		<th width="120">작성일</th>
		<td><input name="bd_write_date1" type="text" value="<?=$bd_write_date1?>" class="cc" size="4">년<input name="bd_write_date2" type="text" value="<?=$bd_write_date2?>" class="cc" size="2">월<input name="bd_write_date3" type="text" value="<?=$bd_write_date3?>" class="cc" size="2">일   조회수 <input name="view_hit" type="text" value="<?=$bd_view_count?>" class="cc" size="5">
		</td>
	</tr>
<? } ?>
...
<? if($wcfg['input_name']) { ?>
	<tr height="25">
		<td width="120" align="center" bgcolor="#F0F0F4"><font color="red"><strong>작성자(필수입력)</strong></font></td>
		<td>&nbsp;<input name="bd_name" type="text" value="<?=$bd_name?>" class="cc">
		</td>
<? if($wcfg['input_pass']) { ?>
		<td align="center" bgcolor="#F0F0F4" width="120"><font color="red"><strong>암호(필수입력)</strong></font></td>
		<td>&nbsp;<input name="bd_pass" type="password" value="" class="cc"></td>
<? } ?>
	</tr>
    <tr>
		<td colspan="4" height="1" bgcolor="#3694D4"></td>
...
<? if($wcfg['input_email']) { ?>
	<tr height="25">
		...
		<td>&nbsp;<input name="bd_email" type="text" class="cc" value="<?=$bd_email?>" size="30">
		</td>
<? } ?>
<? if($wcfg['use_home']) { ?>
    <td align="center" bgcolor="#F0F0F4" width="120"><strong>홈페이지</strong></td>
	<td >&nbsp;<input name="bd_home" type="text" class="cc" value="<?=$bd_home?>" size="30"></td>
<? } ?>
	</tr>
    <tr>
		<td colspan="4" height="1" bgcolor="#3694D4"></td>
...
<? } ?>
	...
	<tr height="25">
		<td align="center" bgcolor="#F0F0F4"><font color="red"><strong>제목(필수입력)</strong></font></td>
		<td colspan="3">&nbsp;<input name="bd_subject" type="text" class="cc" value="<?=$bd_subject?>" size="60"><? if($_mb[mb_level]>9) { ?><br/><input type="radio" name="bd_ext5" value="0" <?if($bd_ext5==0){ echo "checked";}?>>기본색상 <input type="radio" name="bd_ext5" value="1" <?if($bd_ext5==1){ echo "checked";}?>>파란색 <input type="radio" name="bd_ext5" value="2" <?if($bd_ext5==2){ echo "checked";}?>>빨간색<? } ?> <input type="checkbox" name="bd_ext4" value="1" <?if($bd_ext4==1){ echo "checked";}?>>볼드</td>
	</tr>
    <tr>
		<td colspan="4" height="1" bgcolor="#3694D4"></td>
    </tr>
...
<? } ?>
	<tr>
		...
		<td colspan="3"><?=myEditor(1,'../editor','add_form','bd_content','100%','350');?></td>
	</tr>
    <tr>
		<td colspan="4" height="1" bgcolor="#3694D4"></td>
    </tr>
...
?><input type="image" src="<?=$skin_url?>images/new_write.gif" align="absmiddle"  onClick="editor_wr_ok();"><?
		break;
		case 'reply' :
?><input type="image" src="<?=$skin_url?>images/reply.gif" align="absmiddle"  onClick="editor_wr_ok();"><?
		break;
		case 'modify' :
?><input type="image" src="<?=$skin_url?>images/modify.gif" align="absmiddle"  onClick="editor_wr_ok();"><?
		break;
	}
?>
