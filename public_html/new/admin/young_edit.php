   ...
   // 삭제
	...
	if($_SERVER['REQUEST_METHOD']=='POST') {
       ...
       // 파일 업로드
        ...
        @mkdir("../data/young/".$num."/", 0707);
        @chmod("../data/young/".$num."/", 0707);   
	   
	     $school_path="../data/young/".$num."/";
...
<tr >
	...
	<td colspan="3"> <?if($data[school_file1_name]){?>
			  <img src=../data/young/<?=$num?>/<?=$data[school_file1_name]?> width="80" height="30" align="absmiddle">
			  <?}?> 			  
  ...
  <tr>
   ...
   <tr>
	...
	<td colspan="3"><select name="section" class="select2">
        <?=rg_html_option($_const['section2'],$data['section'])?>
        </select></td>
  </tr> 
  <tr>
	<td bgcolor="#BECCDD" height="1" colspan="4"></td>
  ...
  <tr>
	<td colspan="4"><input type="hidden" name="info"><iframe src="young_editor02.php?num=<?=$num?>&control=info" width="100%" height="510" frameborder=0 border=0 scrolling=no name="info_frm"></iframe></td>
	</tr>
  <tr>
	<td bgcolor="#BECCDD" height="2" colspan="4"></td>
  </tr>
<? for ($i=2; $i<=9; $i++){?>
   <tr>
	...
	<td colspan="4" class="a_s_text_title">	<input name='school_file<?=$i?>' type=file class=cc style='width:40%;'>   &nbsp;&nbsp;                     
                         ...
                         <img src="../data/young/<?=$num?>/<?=$data[school_file.$i._name]?>" width="60" height="40" align="absmiddle"> <input name='del_file<?=$i?>' type=checkbox id="del_file<?=$i?>" value='1'>삭제 
                          <?}?>
	</td>
  </tr>
<?}?>
