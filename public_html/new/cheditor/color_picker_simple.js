function drawColor(oName) {
    var safe = oName + "Safe";
    var colText = oName + "Col";
    var sHTML = "";
    ...
    sHTML += "<table border=0 width=350 align=center cellpadding=0 cellspacing=0>" +
        "<tr><td width=45 align=center bgcolor=ghostwhite style='border:#a9a9a9 1 solid'><table border=1 bgcolor=ghostwhite cellpadding=0 cellspacing=0><tr><td id='"+safe+"' width=27 height=27></td></tr></table></td>" +
        "<td>&nbsp;</td><td>";
    ...
    for(var i=1;i<=6;i++)
            ...
            for(var r=0;r<eval("c"+i).length;r++)
                ...
                sHTML += "<td style=\"cursor:hand;background-color:"+colour+";\" width=9 height=6 onclick=\""+safe+".style.background='#"+colour+"';eval("+colText+").innerText='#"+colour+"';\"></td>";
                }
            sHTML += "</tr>";
            }
    sHTML += "</table>";
    ...
    sHTML += "<font size=2>Țȗ :</font> <input type=text id='"+colText+"' contentEditable=true style=\"width:53;height:19px;font:10 arial,sans-serif\" onchange=\""+safe+".style.background=this.value;\">";
    sHTML += "</td></tr>";
    sHTML += "</table>";
    return sHTML;
}
